<template>
    <div class="row">
        <div class="w-100 d-flex justify-content-between">
            <h2> {{ title }} </h2>
            <user-form :loading="submitting" @add-new-user="add($event)"></user-form>
        </div>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    <th scope="col">avatar</th>
                    <th scope="col">email</th>
                    <th scope="col">job</th>
                    <th scope="col">fullname</th>
                    <th scope="col">action</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(dt, key) in users" :key="key">
                    <th scope="row">{{ key + 1 }}</th>
                    <td><img :src="dt.avatar"/></td>
                    <td>{{dt.email}}</td>
                    <td>{{dt.job ? dt.job : 'Nganggur'}}</td>
                    <td>{{dt.first_name + " " +dt.last_name}}</td>
                    <td>
                        <button class="btn btn-danger action" :disabled="submitting" @click="deleteUser(key)">delete</button>
                        <button class="btn btn-warning action" :disabled="submitting" @click="openEdit(dt)">edit</button>
                        <button class="btn btn-success action" :disabled="submitting" @click="view(key)">view</button>
                    </td>
                </tr>
            </tbody>
        </table>
        <dialog-modal @close="openDialogEdit = false" :open="openDialogEdit">
            <user-edit :loading="submitting" @update-user="edit($event)" :user="userEdit"></user-edit>
        </dialog-modal>
        <dialog-modal @close="openDialogView = false" :open="openDialogView">
            <user-view :user="userView"></user-view>
        </dialog-modal>
    </div>
</template>

<style>
    tbody a { padding: 5px;}
    table img { width: 64px; height: 64px; object-fit: cover; object-position: center; }
    table .action {margin: 0 4px;}
</style>

<script>

    import axios from 'axios'
    import config from "./../config";
    import UserForm from './UserForm'
    import UserEdit from './UserEdit'
    import DialogModal from './Dialog'
    import UserView from './UserView'

    export default {
        name: 'Users',
        props: {
            title: {
                type: String,
                default: ""
            }
        },
        data() {
            return {
                openDialogEdit: false,
                openDialogView: false,
                users: [],
                submitting: false,
                userEdit: {},
                userView: {},
                pagination: {
                    page: 0,
                    per_page: 10,
                },
                defaultUserInfo: {
                    email: 'noemail@email.com',
                    avatar: 'data:image/png;base64,'
                }
            }
        },
        components: {
            UserForm,
            UserEdit,
            DialogModal,
            UserView
        },
        created() {
            this.load()
        },
        methods: {
            openEdit(user) {
                this.userEdit = user
                this.openDialogEdit = true
            },
            async load() {
                try {
                    const res = await axios.get(config.users_url, {params: this.pagination})
                    const data = res.data
                    this.users = data.data
                    this.pagination = {
                        page: data.page,
                        per_page: data.per_page,
                        total: data.total,
                        total_pages: data.total_pages,
                    }
                } catch (error) {
                    console.log(err);
                }
            },
            async add(user) {
                this.submitting = true
                try {
                    const res = await axios.post(config.users_url, user)
                    if (res.status === 201) {
                        const newUser = this.fillUserInfo(res.data)
                        this.users = [newUser, ...this.users]
                    }
                    this.submitting = false
                } catch (error) {
                    console.log(error)
                    this.submitting = false
                }
            },
            async deleteUser(index) {
                this.submitting = true
                try {
                    const res = await axios.delete(config.users_url + '/' + this.users[index].id)
                    this.users.splice(index, 1)
                    this.submitting = false
                } catch (error) {
                    console.log(error)
                    this.submitting = false
                }
            },
            async edit(user) {
                this.submitting = true
                try {
                    const res = await axios.put(config.users_url + '/' + user.id)
                    const editIndex = this.users.findIndex(u => u.id === user.id)
                    this.users[editIndex] = user
                    this.submitting = false
                    this.openDialogEdit = false
                } catch (error) {
                    console.log(error)
                    this.submitting = false
                }
            },
            view(index) {
                this.userView = this.users[index]
                this.openDialogView = true
            },
            fillUserInfo(user) {
                !user.email ? user.email = this.defaultUserInfo.email : null
                !user.avatar ? user.avatar = this.defaultUserInfo.avatar : null
                if (user.name) {
                    user.first_name = user.name
                    user.last_name = ''
                }
                return user
            }
        },
    }
</script>