import Vue from 'vue'
import Vuex from 'vuex'
import axios from 'axios'
import config from './config'

Vue.use(Vuex)

export default new Vuex.Store({
    state: {
        users: [],
        defaultPagination: {
            page: 1,
            per_page: 5,
        },
        pagination: {},
        active: {
            show: false,
            type: null,
            user: null
        },
        submitting: false,
        fetching: false,
        defaultUserInfo: {
            email: '-',
            avatar: 'data:image/png;base64,'
        }
    },

    mutations: {
        setData(state, {data, page, per_page, total, total_pages}) {
            state.users = data
            state.pagination = {page, per_page, total, total_pages}
        },
        addNewUser(state, newUser) {
            !newUser.email ? newUser.email = state.defaultUserInfo.email : null
            !newUser.avatar ? newUser.avatar = state.defaultUserInfo.avatar : null
            if (newUser.name) {
                newUser.first_name = newUser.name
                newUser.last_name = ''
            }
            state.users = [newUser, ...state.users]
        },
        setSubmitting(state, payload) {
            state.submitting = payload
        },
        removeUserFromList(state, index) {
            state.users.splice(index, 1)
        },
        editLocalUser(state, payload) {
            state.users[payload.index] = payload.user
        },
        setActiveUser(state, payload) {
            state.active = payload
            state.active.show = true
        },
        setFetching(state, payload) {
            state.fetching = payload
        },
        removeActiveUser(state) {
            state.active = {show: false, type: null, user: null}
        }
    },

    actions: {
        async fetchUsers({state, commit}, payload) {
            commit('setFetching', true)
            if (!payload) payload = state.defaultPagination
            if (!payload.page) payload.page = state.defaultPagination.page
            if (!payload.per_page) payload.per_page = state.defaultPagination.per_page
            try {
                const res = await axios.get(config.users_url, {params: payload})
                if (res.status === 200) {
                    commit('setData', res.data)
                }
                commit('setFetching', false)
                return res
            } catch (error) {
                commit('setFetching', false)
                return error
            }
        },

        async addUser({commit}, payload) {
            commit('setSubmitting', true)
            try {
                const res = await axios.post(config.users_url, payload)
                if (res.status === 201) {
                    commit('addNewUser', res.data)
                }
                commit('setSubmitting', false)
                return res
            } catch (error) {
                commit('setSubmitting', false)
                return error
            }
        },

        async deleteUser({commit, dispatch}, id) {
            commit('setSubmitting', true)
            const index = await dispatch('getUserIndex', id)
            try {
                const res = await axios.delete(config.users_url + '/' + id)
                if (res.status === 204) {
                    commit('removeUserFromList', index)
                }
                commit('setSubmitting', false)
                return res
            } catch (error) {
                commit('setSubmitting', false)
                return error
            }
        },

        async editUser({commit, dispatch}, user) {
            commit('setSubmitting', true)
            const index = await dispatch('getUserIndex', user.id)
            try {
                const res = await axios.put(config.users_url + '/' + user.id)
                if (res.status === 200) {
                    commit('editLocalUser', {index, user})
                    commit('setSubmitting', false)
                    dispatch('closeDialog')
                }
                return res
            } catch (error) {
                commit('setSubmitting', false)
                return error
            }
        },

        openDialog({commit}, payload) {
            if (!payload.type) payload.type = 'view'
            if (payload.user) commit('setActiveUser', payload)
        },

        closeDialog({commit}) {
            commit('removeActiveUser')
        },

        getUserIndex({state}, id) {
            return state.users.findIndex(user => user.id === id)
        }
    }
})